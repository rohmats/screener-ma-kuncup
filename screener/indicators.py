...
from screener.config import MA_PERIODS, VOL_ROLLING_PERIOD, MA_TREND_PERIOD
...
def calculate_volatility(df: pd.DataFrame, period: int = VOL_ROLLING_PERIOD) -> pd.DataFrame:
...
def calculate_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = calculate_moving_averages(df, MA_PERIODS + [MA_TREND_PERIOD])
